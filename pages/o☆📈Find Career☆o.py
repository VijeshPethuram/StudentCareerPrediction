...
loaded_model=pickle.load(open('C://Users//sanch//OneDrive//Desktop//prab//Career_trained_model.sav','rb'))
loaded_models=pickle.load(open('C://Users//sanch//OneDrive//Desktop//prab//Stream_trained_model.sav','rb'))
...
with open("C://Users//sanch//OneDrive//Desktop//multipage//reply.txt", "r") as file:
    ...
    canworklong = 1 if file.readline().strip()=='Yes' else 0 
    selflearning = 1 if file.readline().strip()=='Yes' else 0
    Extracourses = 1 if file.readline().strip()=='Yes' else 0
    talenttests = 1 if file.readline().strip()=='Yes' else 0
    olympiads = 1 if file.readline().strip()=='Yes' else 0
    ...
    HigherStudies = 1 if file.readline().strip()=='Higher Studies' else 0
    Takeninputsfromseniors = 1 if file.readline().strip()=='Yes' else 0
    interestedingames = 1 if file.readline().strip()=='Yes' else 0
    ...
    if temp=='prayer books':
        ...
        booktype=2
    Tuff = 1 if file.readline().strip()=='Stubborn' else 0
    Management = 1 if file.readline().strip()=='Technical' else 0
    hardsmart = 1 if file.readline().strip()=='Smart Worker' else 0
    workedinteams = 1 if file.readline().strip()=='Yes' else 0
    Introvert = 1 if file.readline().strip()=='Yes' else 0
...
if st.button('Check my career'):
      res=careerpred([Marks,Communicationskills,Hourswork,Logicalqr,publicspeak,canworklong,selflearning,Extracourses,talenttests,olympiads,readingwritingskills,memorycapabilityscore,HigherStudies,Takeninputsfromseniors,interestedingames,Tuff,Management,hardsmart,workedinteams,Introvert,temp1,temp])
      stream=careerpreds([Marks,Communicationskills,Hourswork,Logicalqr,publicspeak,canworklong,selflearning,Extracourses,talenttests,olympiads,readingwritingskills,memorycapabilityscore,HigherStudies,Takeninputsfromseniors,interestedingames,Tuff,Management,hardsmart,workedinteams,Introvert,temp1,temp])
      import csv
